...
-- depends_on: {{ source('sage200_etl_frl', '_airbyte_raw_purchase_posted_transactions') }}
select
    ...
    {{ json_extract_scalar('_airbyte_data', ['supplier_id'], ['supplier_id']) }} as supplier_id,
    {{ json_extract_scalar('_airbyte_data', ['posted_date'], ['posted_date']) }} as posted_date,
    {{ json_extract_scalar('_airbyte_data', ['vat_adjustment_doc_expected'], ['vat_adjustment_doc_expected']) }} as vat_adjustment_doc_expected,
    _airbyte_ab_id,
    _airbyte_emitted_at,
    {{ current_timestamp() }} as _airbyte_normalized_at
from {{ source('sage200_etl_frl', '_airbyte_raw_purchase_posted_transactions') }} as table_alias
