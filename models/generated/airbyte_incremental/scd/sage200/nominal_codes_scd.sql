{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_frl.nominal_codes_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_frl.nominal_codes_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    select
        *
    from {{ ref('nominal_codes_stg')  }}
    -- nominal_codes from {{ source('sage200_etl_frl', '_airbyte_raw_nominal_codes') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nominal_codes_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nominal_codes_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nominal_codes_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
