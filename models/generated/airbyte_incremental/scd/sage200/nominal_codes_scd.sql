{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg.nominal_codes_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg.nominal_codes_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- nominal_codes from {{ source('sage200_etl', '_airbyte_raw_nominal_codes') }}
    where 1 = 1
