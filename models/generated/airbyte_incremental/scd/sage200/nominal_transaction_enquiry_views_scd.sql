{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='nominal_transaction_enquiry_views'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_transaction_enquiry_views')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_transaction_enquiry_views')) }}
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_frl.nominal_transaction_enquiry_views_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_frl.nominal_transaction_enquiry_views_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('nominal_transaction_enquiry_views_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('nominal_transaction_enquiry_views_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nominal_transaction_enquiry_views_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nominal_transaction_enquiry_views_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nominal_transaction_enquiry_views_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      {{ adapter.quote('id') }} as _airbyte_start_at,
      lag({{ adapter.quote('id') }}) over (
        partition by cast({{ adapter.quote('id') }} as {{ dbt_utils.type_string() }})
        order by
            {{ adapter.quote('id') }} is null asc,
            {{ adapter.quote('id') }} desc,
