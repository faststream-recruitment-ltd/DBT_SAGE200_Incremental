{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_frl.customers_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_frl.customers_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- customers from {{ source('sage200_etl_frl', '_airbyte_raw_customers') }}
    where 1 = 1
