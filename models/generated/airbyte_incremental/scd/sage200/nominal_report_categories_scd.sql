{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='nominal_report_categories'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_report_categories')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_report_categories')) }}
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_frl.nominal_report_categories_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_frl.nominal_report_categories_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('nominal_report_categories_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('nominal_report_categories_stg')  }}
    -- nominal_report_categories from {{ source('sage200_etl_frl', '_airbyte_raw_nominal_report_categories') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nominal_report_categories_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nominal_report_categories_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nominal_report_categories_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
