{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='nominal_codes_financ_years_period_balances'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_codes_financ_years_period_balances')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('nominal_codes_financ_years_period_balances')) }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nominal_codes_financ_years_period_balances_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nominal_codes_financ_years_period_balances_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nominal_codes_financ_years_period_balances_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(date_time_updated) over (
        partition by period_balance_id
        order by
